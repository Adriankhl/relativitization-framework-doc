...
\KwInput{\\
  ...
  $L_{TXYZ}$ 4D array of list of player data
}
\KwOutput{\\
  \Indp
  $M$ map from player id to player data\\
  $\Lambda_{XYZ}$ 3D array of map from group id to list of player id
}

Initialize empty $M$ and $\Lambda_{XYZ}$\;
\ForEach{$X_j, Y_j, Z_j$}{
  $T_j \gets T_i - \tau(\overrightarrow{U_i}, (X_j, Y_j, Z_j))$\;
  \ForEach{data of player $k$ in $L_{T_j X_j Y_j Z_j}$}{
    \If{$M$ has key $k$}{
      \If{$T$ of $M[k]$ < $T_k$}{
        Replace $M[k]$ by this new data\;
      }
      \Else{
...
\KwOutput{\\
  \Indp
  $M'$ map from player id to player data\\
  $\Lambda'_{XYZ}$ 3D array of map from group id to list of player id
}

$M' \gets M$\;
$\Lambda'_{XYZ} \gets \Lambda_{XYZ}$\;
\ForEach{data of player $k$ in $L_{T_j X_j Y_j Z_j}$ where $g(\overrightarrow{u_k}, \overrightarrow{U_k}) = g_i$}{
  \If{$T$ of $M[k]$ < $T_k$}{
    Replace $M'[k]$ by this new data\;
...
\KwInput{\\
  \Indp
  $M_\textrm{current}$ map from player id to current player data\\
  $M_{\textrm{command}}$ map from player id to list of commands
}
...
\ForEach{data of player $i$ in $M_\textrm{current}$}{
  ...
  \If{coordinates or group is new}{
    Save old coordinates to history $H_i$\;
  }
  \ForEach{$(T_i', X_i', Y_i', Z_i')$ in $H_i$}{
    Remove from $H_i$ if $T_{\textrm{current}} - T' > \Delta T_{\textrm{max}}$\;
  }
  Save the new data to $L_{X_i Y_i Z_i}$\;
  \ForEach{$(T_i', X_i', Y_i', Z_i')$ in $H_i$}{
    Find the old player data from $L_{T_i' X_i' Y_i' Z_i'}$\;
    Add the old player data to $L_{X_i' Y_i' Z_i'}$\;
  }
}
